...
def file_level_check(is_fw_file,is_multiline,is_dataclean,csv_filename,delimiter_value,input_file_prefix,snap_dt,run_id,src_name,file_name,meta_data_df,rowcountvariance_df, df, bad_rows_cnt, df_header_count,**kwargs):
    ...
    try:
        ...
        fetch_trgt_tbl="select distinct case when trgt_tbl_nm is null then file_prefix_nm else trgt_tbl_nm end as trgt_tbl_nm from {table} where lower(file_prefix_nm) = '{prefix}' and  src_sys_nm='{src_name}' ".format(table = app_config.file_expctd_list,prefix = input_file_prefix.lower(),src_name=src_name)

        trgt_tbl=frmspark.execute_query_ctrl_tbl_api(fetch_trgt_tbl).collect()[0][0]
        ...
        rule=[{'checks':x['file_chck_typ'],'arguments':x['arguments']} for x in spark.sql("select distinct file_chck_typ,arguments,priority from {} where file_prefix_nm='{}' order by priority".format(app_config.file_checklist, input_file_prefix)).collect()]
        ...
        if checkAll:
            if is_fw_file:
                ...
                col_length_query = "select col_length from {} where lower(file_prefix_nm) = '{}' order by col_seq asc".format(app_config.file_schema_info,input_file_prefix.lower())
                col_length_df = frmspark.execute_query(col_length_query)
                col_length_list = [int(x['col_length']) for x in col_length_df.collect()]
                input_regex = ""
                ...
                multiLine_json_obj = json.loads(frmspark.execute_read_ctrl_tbl_api(table = app_config.mapping_table ).filter(F.col('group_name')=='listOfMultilineFiles').select('val1').rdd.flatMap(lambda x: x).collect()[0])
            ...
            # print("final df ")
            # write_df.show()
            intra_day_query="select coalesce(lower(trim(intraday_flag)),'n') as intraday_flag from  {}  where lower(file_prefix_nm) = '{}' and trgt_tbl_nm= '{}'".format(app_config.file_expctd_list,input_file_prefix.lower(
            ),trgt_tbl)
            intraday_res = frmspark.execute_query(intra_day_query)
            intraday_flag=intraday_res.select('intraday_flag').distinct().collect()[0][0]
        ...
        type_casting_query = "select distinct typecasting_flag from {file_expcted_list_view} where lower(file_prefix_nm) = lower('{prefix}') and lower(src_sys_nm) = '{src_sys_nm}' ".format(file_expcted_list_view = app_config.file_expctd_list,  prefix = input_file_prefix , src_sys_nm=src_name.lower())

        type_casting_df = frmspark.execute_query(type_casting_query)
        type_casting_value = type_casting_df.first()[0]
...
def preprocessing(src_sys_nm, input_file_prefix):
    ...
    try:
        ...
        file_type_query = "select file_type,part_file_flag from {} where lower(file_prefix_nm) = '{}'".format(app_config.file_expctd_list,input_file_prefix.lower())
        file_type_result = frmspark.execute_query(file_type_query).collect()[0]
        is_fw_file = False
        ...
        delimiter_value_query = """
            SELECT DISTINCT hexacodes_flag, field_delimiter
            FROM {table}
            WHERE lower(file_prefix_nm) = '{prefix}'
        """
        delimiter_value_query = delimiter_value_query.format(
            table=app_config.file_expctd_list,
            prefix=input_file_prefix.lower(),
        )
        delimiter_dataframe = frmspark.execute_query_ctrl_tbl_api(delimiter_value_query) #Changes for Jira::WMIT-5794 | to reduce number of partitions in dataframe
        delimiter_data=delimiter_dataframe.collect()
        hexacodes_flag = delimiter_data[0]['hexacodes_flag']
        field_delimiter =delimiter_data[0]['field_delimiter']
        ...
        for num_rows in range(count):
            try:
                ...
                if is_fw_file == True:
                    ...
                    fetch_col_head_flag="select distinct case when col_header_flag is null then 'Y' else col_header_flag end as col_header_flag from {table} where lower(file_prefix_nm) = '{prefix}' ".format(table = app_config.file_expctd_list,prefix = input_file_prefix.lower())

                    col_header_flag=frmspark.execute_query_ctrl_tbl_api(fetch_col_head_flag).collect()[0][0]
                    empty_file_flag = 0
                    ...
                    if (rdd_datafile.isEmpty() == False):
                            ...
                            try:
                                file_content_format = frmspark.execute_read_ctrl_tbl_api(table = app_config.file_checklist ).filter((F.col('file_prefix_nm')==input_file_prefix ) & (F.col('file_chck_typ')=='fileContentCheck')).select('arguments').rdd.flatMap(lambda x: x).collect()[0]
                            except:
                            ...
                            fetch_col_head_flag="select distinct case when col_header_flag is null then 'Y' else col_header_flag end as col_header_flag from {table} where file_prefix_nm = '{prefix}' ".format(table = app_config.file_expctd_list,prefix = input_file_prefix )
                    ...
                    total_col_len_query = "select SUM(col_length) from {} where lower(file_prefix_nm) = '{}'".format(app_config.file_schema_info,input_file_prefix.lower())
                    total_col_length = int((frmspark.execute_query(total_col_len_query)).collect()[0][0])
                    ...
                    file_level_check(is_fw_file,is_multiline,is_dataclean,csv_filename,delimiter_value,input_file_prefix,snap_dt,run_id,src_name,file_name,meta_data_df,rowcountvariance_df, final_df, bad_rows_cnt, header_count, last_row = '', mapping_list='', col_list=col_list, data_headers='', top10rows = '',data_count= data_count,file_content_dict = file_content_dict,part_file_flag = part_file_flag)
                else:
                    try:
                        json_obj=json.loads(frmspark.execute_read_ctrl_tbl_api(table = app_config.mapping_table ).filter(F.col('group_name')=='data_clean_configuration').select('val1').rdd.flatMap(lambda x: x).collect()[0])
                        multiline_files_list = json.loads(frmspark.execute_read_ctrl_tbl_api(table = app_config.mapping_table ).filter(F.col('group_name')=='listOfMultilineFiles').select('val1').rdd.flatMap(lambda x: x).collect()[0])
                        xml_obj=json.loads(frmspark.execute_read_ctrl_tbl_api(table = app_config.mapping_table ).filter(F.col('group_name')=='xml_configuration').select('val1').rdd.flatMap(lambda x: x).collect()[0])
                        xml_config = False
                        is_multiline = False
                        is_dataclean = False
                        quote_flag = None
